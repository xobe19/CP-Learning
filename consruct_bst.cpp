...
BinaryTreeNode* construct_bst(vector<int> &v, int a , int b) {
    cout << a << " " << b << endl;
if(a > b) return NULL;
int mid = (b + a) / 2;
auto root = new BinaryTreeNode(v[mid]);
root -> left =construct_bst(v,a, mid - 1);
root -> right = construct_bst(v, mid + 1, b);
return root;
}
...
auto ans = construct_bst(v, 0, v.size() - 1);
